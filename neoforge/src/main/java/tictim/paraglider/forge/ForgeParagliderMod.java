...
public class ForgeParagliderMod extends ParagliderMod{
	...
	private final ForgeCommonConfig commonCfg;
	private final ForgeContents contents;
	private final ForgeConfig config;

	public ForgeParagliderMod(IEventBus modEventBus) {
		if (FMLEnvironment.dist == Dist.CLIENT) {
			proxy = new ClientProxy(modEventBus);
		} else {
			proxy = new CommonProxy(modEventBus);
		}

		commonCfg = new ForgeCommonConfig(modEventBus);
		config = new ForgeConfig(modEventBus);
		contents = new ForgeContents(modEventBus);
	}
